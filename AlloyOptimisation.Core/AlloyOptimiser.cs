...
public class AlloyOptimiser
{
    ...
    /// It's not optimised and would scale awfully with a large amount of configuration data, but for the purpose of this exercise it was deemed sufficient.
    /// </summary>
    ...
    static void GridSearch(List<AlloyComponentConfig> alloyComponents, int index, Alloy currentAlloy, ref Alloy? optimumAlloy, decimal costConstraint)
    {
        ...
        {
            var success = currentAlloy.TryUpdateComponent(alloyComponent.Element, atomicPercentage);
            if (!success) continue; // Component percentage would bring total composition above 100%
